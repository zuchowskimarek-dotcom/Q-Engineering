...
async function fixVariables() {
    try {
        ...
        // Solution: Use ${team:sqlstring} inside variable queries to ensure quotes around UUIDs
        // Syntax failure: WHERE id IN ($team) -> WHERE id IN (abc-123) -> ERROR
        // Correct check: WHERE id IN (${team:sqlstring}) -> WHERE id IN ('abc-123') -> OK
        ...
        const projectVar = {
            ...
            "name": "project",
            "options": [],
            // FIX: Use ${team:sqlstring}
            "query": "SELECT DISTINCT p.id as value, p.name as text FROM \"Project\" p JOIN \"TeamProject\" tp ON p.id = tp.\"projectId\" WHERE tp.\"teamId\" IN (\${team:sqlstring}) AND p.\"isSelected\" = true ORDER BY p.name",
            "refresh": 1,
            "regex": "",
            "skipUrlSync": false,
            "sort": 1,
        ...
        const memberVar = {
            ...
            "options": [],
            // FIX: Use ${team:sqlstring}
            "query": "SELECT DISTINCT pers.id as value, pers.name as text FROM \"Person\" pers JOIN \"TeamMembership\" tm ON pers.id = tm.\"personId\" WHERE tm.\"teamId\" IN (\${team:sqlstring}) ORDER BY pers.name",
            "refresh": 1,
            "regex": "",
            "skipUrlSync": false,
            "sort": 1,
        ...
        console.log('Dashboard variables fixed successfully (v2 - quoted):', updateResponse.data.status);
